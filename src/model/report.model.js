...
const historyQuery = `SELECT history.invoice, history.cashier, history.customer_id, history.order_date, menu.name AS menu_name, menu.image_path, menu.price, menu_order_history.quantity AS menu_quantity, history.amount AS total_amount FROM history JOIN menu_order_history ON menu_order_history.invoice = history.invoice JOIN menu ON menu_order_history.menu_id = menu.id JOIN users ON users.id = history.customer_id`;
const historyModel = {
